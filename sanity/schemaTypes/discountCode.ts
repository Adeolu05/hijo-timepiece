...
/**
 * Studio-ready `discountCode` document. Matches GROQ in `src/lib/sanity.ts` and mapping in `src/lib/discountCodes.ts`.
 * Keep in sync with `hijo/schemaTypes/discountCode.ts`.
 */
      ...
      const offer =
        discountType === "amount" && typeof amountOffNgn === "number"
          ...
          : typeof percentOff === "number"
            ...
            : "";
      const until =
        typeof validUntil === "string" && validUntil.length >= 10
          ? `until ${validUntil.slice(0, 10)}`
          : "";
      return {
