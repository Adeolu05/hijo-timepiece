...
export function ProductCard({ watch }: ProductCardProps) {
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden bg-surface-container-low mb-4 luxury-shadow ring-1 ring-transparent group-hover:ring-outline-variant/35 transition-[transform,box-shadow,ring-color] duration-500">
        <img
          src={watch.image}
          alt={watch.name}
      ...
      <div className="border-t border-outline-variant/20 pt-3 md:pt-3.5 flex flex-col items-center text-center">
        <span className="wide-label !text-[8px] text-on-surface-variant/55 mb-1.5">{statusLabel}</span>
        <h3 className="font-headline text-[1.7rem] md:text-[1.95rem] text-primary tight-headline mb-1 group-hover:text-secondary transition-colors duration-500">
          {watch.name}
        </h3>
        <span className="block wide-label !text-[9px] tracking-[0.16em] text-on-surface-variant/82 mb-2.5 italic px-2">
          {supportingLine}
        </span>
        <div className="w-full max-w-[18rem] flex items-center justify-center gap-3 border-t border-outline-variant/20 pt-2">
